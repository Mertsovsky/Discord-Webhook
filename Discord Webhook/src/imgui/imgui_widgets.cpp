...
bool ImGui::CloseButton( ImGuiID id, const ImVec2 &pos, float radius ) {
    ...
    // We intentionally allow interaction when clipped so that a mechanical Alt,Right,Validate sequence h a window.
    ...
    if ( hovered )
        window->DrawList->AddRectFilled( bb.Min, bb.Max, GetColorU32( held ? ImGuiCol_ButtonActive : ImGuiCol_ButtonHovered ) );

    float cross_extent = ( radius * 0.7071f ) - 1.0f;
    ImU32 cross_col = GetColorU32( hovered ? ImGuiCol_Text : ImGuiCol_TextDisabled );
...
// Button to hide a window
bool ImGui::HyphenButton( ImGuiID id, const ImVec2 &pos, float size ) {
    ImGuiContext &g = *GImGui;
    ImGuiWindow *window = g.CurrentWindow;

    const ImRect bb( pos - ImVec2( size, size ), pos + ImVec2( size, size ) );
    bool is_clipped = !ItemAdd( bb, id );
    ...
    float cross_extent = ( size * 0.7071f ) - 1.0f;
    ImU32 cross_col = GetColorU32( hovered ? ImGuiCol_Text : ImGuiCol_TextDisabled );
    center -= ImVec2( 0.5f, 0.5f );
    window->DrawList->AddLine( center - ImVec2( cross_extent, 0 ), center + ImVec2( cross_extent, 0 ), cross_col, 2.f );

    return pressed;
}

bool ImGui::TextButton( const char *label, const ImVec2 &pos, float size ) {
    ImGuiContext &g = *GImGui;
    ImGuiWindow *window = g.CurrentWindow;
    ...
    ImGui::PushStyleColor( ImGuiCol_Text, cross_col );
    RenderText( center - ImVec2( cross_extent, cross_extent + 0.5f ), label );
    ImGui::PopStyleColor();
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float square_sz = g.FontSize + 3.f;
    const ImVec2 pos = window->DC.CursorPos;
    ...
    it_hover->second = ImClamp( it_hover->second + ( 0.2f * ImGui::GetIO().DeltaTime * ( hovered ? 6.f : -6.f ) ), 0.0f, 0.12f );
    ...
    ImColor check_col = g.Style.Colors[ImGuiCol_CheckMark];
    ...
    if ( mixed_value ) {
        ...
        check_col.Value.w = it_filled->second;
        ...
        RenderCheckMark( window->DrawList, check_bb.Min + ImVec2( pad, pad ), ImColor( 255, 255, 255, ( int )( 255 * it_filled->second ) ), square_sz - pad * 2.0f );
    }

    if ( g.LogEnabled )
        LogRenderedText( &total_bb.Min, *v ? "[x]" : "[ ]" );
    if ( label_size.x > 0.0f )
        RenderText( ImVec2( check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + 1.5f ), label );
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        {
            // Draw
            window->DrawList->AddLine(bb.Min, ImVec2(bb.Max.x, bb.Min.y), GetColorU32(ImGuiCol_Separator), 1.7f);
            if (g.LogEnabled)
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (BeginPopup("picker", ImGuiWindowFlags_AlwaysAutoResize))
        {
            picker_active_window = g.CurrentWindow;

            const auto flags = ImGuiColorEditFlags_PickerHueBar | ImGuiColorEditFlags_NoDragDrop | ImGuiColorEditFlags_NoInputs |
                ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_NoSidePreview;
...
bool ImGui::CollapsingHeader(const char* label, bool* p_visible, ImGuiTreeNodeFlags flags)
{
    ...
    if (p_visible != NULL)
    {
        ...
        if (CloseButton(close_button_id, ImVec2(button_x, button_y), g.FontSize * 0.5f + 1))
            *p_visible = false;
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
    ...
    if (close_button_visible)
    {
        ...
        if (CloseButton(close_button_id, button_pos, g.FontSize * 0.5f + 1 ))
